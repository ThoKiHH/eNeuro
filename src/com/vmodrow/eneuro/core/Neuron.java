...
package com.vmodrow.eneuro.core;

import com.vmodrow.eneuro.function.activation.ActivationFunction;
import com.vmodrow.eneuro.function.activation.ActivationFunctionType;
import com.vmodrow.eneuro.function.input.InputFunction;
import com.vmodrow.eneuro.function.input.InputFunctionType;

import java.util.ArrayList;

/**
 * Created by Vincent Modrow on 21/02/2017.
 * com.vmodrow.eneuro.core
 */
public class Neuron implements java.io.Serializable {
    ...
    private ArrayList<Connection> connectionOut;
    private ArrayList<Connection> connectionIn;


    /**
     * Inputfunction == SumByWeight();
     * ActivationFunction == TANH();
     */
    public Neuron() {
        this(InputFunctionType.SumByWeight.getTypeClass(), ActivationFunctionType.TANH.getTypeClass());
    }
    ...
    public Neuron(InputFunction inputFunction, ActivationFunction activationFunction) {
        ...
        this.connectionIn = new ArrayList<>();
        this.connectionOut = new ArrayList<>();
        this.inputValue = 0d;
        this.outputValue = 0d;
    }
     ...
     */
    public void setActivationFunction(ActivationFunctionType activationFunctionType) throws IllegalArgumentException {
        if (activationFunctionType != null) {
            this.activationFunction = activationFunctionType.getTypeClass();
     ...
     * @throws IllegalArgumentException
     */
    public void setActivationFunction(ActivationFunction activationFunction) throws IllegalArgumentException {
    ...
    /**
     * @param inputFunctionType
     * @throws IllegalArgumentException
     */
    public void setInputFunction(InputFunctionType inputFunctionType) throws IllegalArgumentException {
        if (inputFunctionType != null) {
            this.inputFunction = inputFunctionType.getTypeClass();
        } else throw new IllegalArgumentException();
    }
     ...
     * @throws IllegalArgumentException
     */
    public void setInputFunction(InputFunction inputFunction) throws IllegalArgumentException {
    ...
    public void addInputConnection(Neuron fromNeuron) {
        Connection cn = new Connection(fromNeuron, this);
        connectionIn.add(cn);
    }
    ...
    public void addOutputConnection(Neuron toNeuron) {
        if (toNeuron == null) throw new IllegalArgumentException();
        if (hasConnectionToNeuron(toNeuron)) return;
        else this.connectionOut.add(new Connection(this, toNeuron));

    }

    /**
     * Calculates the OutputValue, which will be loaded by the next connected Neuron.
     */
    private void calculateOutput() {
        outputValue = this.activationFunction.getOutput(inputValue);
    }
    ...
    private void calculateInput() {
        if (!connectionIn.isEmpty()) {
            inputValue = this.inputFunction.getOutput(connectionIn.toArray(new Connection[0])); //Convert ArrayList to Connection[]
        } else {
            inputValue = 0d;
        }
    }
    ...
    /**
     * Checks if @param checkNeuron has a @see Connection to this @see Neuron
     *
     * @return
     */
    public boolean hasConnectionToNeuron(Neuron checkNeuron) {
        for (Connection c : connectionOut.toArray(new Connection[0])) {
            if (c.getToNeuron() == checkNeuron) return true;
        }
        return false;
    }
