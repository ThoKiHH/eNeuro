 ...
 */
public class Log extends ActivationFunction {
    ...
    public double getOutput(double netinput) {
        this.output = 1 / (1 + Math.pow(Math.E, -netinput));
        return this.output;
    }

    /**
     * @param netinput
     * @return f'(x) = e^x / (e^x+1)^2
     */
    @Override
    public double getDeriverative(double netinput) {
        this.gradient = (Math.pow(Math.E, netinput) / Math.pow((Math.pow(Math.E, netinput) + 1), 2));
